...
\newcommand{\rlblXlabB}{fraction of anchors occluded}
\newcommand{\rlblYlabB}{gain, F@2\,\%}
\newcommand{\rlblZero}{no anchor on\\the hidden face}
...
input image: it is resampled with the noise seed, shifting by $130^\circ$ in median
...
$+8.05$ when the frame is given: frame estimation consumes 60--74\% of the achievable
gain, and 91\% of the run-to-run variance. Anchoring the frame instead by
...
model: six oriented contacts reconstructed by Poisson reach $19.4$ \Fs{} against
...
\begin{enumerate}
  ...
  from its input}: across three noise seeds on the same image, it moves by
  $130^\circ$ in median on 42 objects, with asymmetric objects no more stable than
  ...
  $+3.21$ realised against $+8.05$ with the frame given
  ...
  \item A \textbf{decomposition of run-to-run variance}, showing that 91\% of it comes
  from frame re-estimation and not from the guidance, and three evaluation pitfalls
...
discrepancy is $131.8^\circ$ for asymmetric objects and $134.4^\circ$ for symmetric
ones; 1 of 16 asymmetric objects is stable below $30^\circ$, 0 of 26 symmetric ones;
the asymmetry/stability correlation is $-0.124$. Repeating the registration moves
these medians by several degrees --- it samples points at random, and on a solid of
revolution the rotation it finds is arbitrary --- but not the split-level conclusion:
the stable counts and the absence of any asymmetry--stability relation are unchanged
between runs. The split by symmetry rules out a
...
55$\,s per sample at 20 steps on an M2 Max --- no remote compute.
...
cast from outside, first hits only), restriction to the camera-occluded face, and
...
distance, connectivity, and recall split by camera visibility.
...
\midrule
Mean gain, frame given (oracle) & $+8.05$ \\
\bottomrule
\end{tabular}
\end{table}
...
$+3.21$ against $+8.05$ over 42 objects, and $+1.79$ against $+6.77$ under the
palpation model on five. \textbf{Frame estimation absorbs 60--74\% of what constraint
...
--- $0.26$ when both conditions come from the same campaign and therefore share the
oracle frame, so that placement noise cancels in the difference, and $0.91$ when they
come from different campaigns, which share no unanchored pass. Colour marks
...
cases of the grasp-strategy campaign. The least-squares fit slopes the wrong way, and
the three cases in which the hand touches \emph{nothing} the camera cannot already see
(green) are among the largest gains measured.}
\label{fig:results}
\end{figure*}
...
Using the grasp simulator, three strategies place the same six anchors with $83\%$,
$70\%$ and $37\%$ of them on the occluded face. Their gains are $+1.73$, $+1.37$ and
$+2.55$: the strategy that touches the hidden face \emph{least} gains \emph{most}. Over
45 cases the correlation between occluded fraction and gain is $r = -0.145$, slightly
the wrong way (Fig.~\ref{fig:results}b). The decisive case is unambiguous: on the
power drill, the strategy that places \textbf{zero} anchors on the occluded face ---
touching only what the camera already sees --- gains $+5.68$ on average. Sparse
...
\begin{table}[t]
\centering
\caption{Six oriented contacts, with and without a generative prior.}
\label{tab:poisson}
\begin{tabular}{lr}
\toprule
Poisson reconstruction~\cite{poisson}, 6 oriented contacts & 19.41 \Fs \\
Poisson reconstruction, 18 contacts (3 grasps) & 20.10 \\
\midrule
...
34\% of what the image alone already achieves. Sparse measurements are a poor
...
\textbf{Guidance amplifies nothing: 91\% of the variance of an anchored score comes
from re-estimating where the anchors go.} Re-scoring one saved mesh ten times accounts
for a further $0.32$, so generation itself contributes under $0.4$. The frame problem
of Sec.~\ref{sec:frame} therefore appears twice --- it consumes most of the mean gain,
and it dominates the variance.
...
share an unanchored pass. The two families carry standard errors of $\approx 0.26$ and
$0.91$ over 15 cases (Fig.~\ref{fig:results}a). We nonetheless base every claim on the
sign test: the frozen-frame $\sigma$ rests on eight repetitions of one object, and its
95\% confidence interval, $[0.46, 1.42]$, is wide enough that the smaller effects move
between $1.8$ and $5.5$ standard errors depending on where the truth lies.
...
\textbf{An inherited defect.} The renderer normalises each object to a maximum
dimension of $1.3$, while every occlusion computation normalises it to $2.0$ with the
camera left in place --- the object is $1.54\times$ too large relative to the camera
when visibility is decided. On the normal-based test that labels the six grasp anchors
this changes \emph{nothing}: not one of the 45 cases moves, and $r$ stays at $-0.145$.
On the hidden-point-removal split used for the visible/occluded recall it shifts the
visible fraction by $0.7$ to $5.7$ percentage points, so those columns carry that
error.
...
noise, so no estimator can learn it. Estimating it anyway costs 60--74\% of the
achievable gain and accounts for 91\% of the run-to-run variance.
