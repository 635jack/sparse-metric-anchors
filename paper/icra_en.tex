...
\newcommand{\rlblVis}{anchors on the visible face instead}
...
the injection of such sparse measurements into a pretrained prior. Of the measurements
themselves, only how widely they cover the surface matters much, and a grasp does not
choose that; their number, their normals and which face of the object they touch matter
little. What limits them once drawn is the \emph{output frame} of the generator. On a billion-parameter wavelet-latent
...
training, improves F-Score@2\% by $+3.2$ on 42 YCB objects (two independent campaigns:
$+3.17$, $p = 9.5\times10^{-3}$, and $+3.21$, $p = 3.1\times10^{-4}$; sign test),
against $+7.9$ when the frame is given: frame estimation consumes 60\% of the
achievable gain, and, once the frame is frozen, the anchored pass is as reproducible as the
...
frame adds $+0.38$. Across seven controlled manipulations and a $2\times2$ design over
126 paired cases, what an anchor set needs is spatial spread and placement accuracy: at
equal count, uniform coverage gains twice what eight contact patches gain, while count,
surface normals and which face they lie on matter little --- 128 depth-like points on
the visible face gain $+15.6$ against $+7.9$ for eight tactile patches on the hidden
face. Five alternative fusion formulations that treat the measurements as a
...
decides how much they help has little to do with what a hand can vary: not their number
over a $16\times$ range, not whether they carry a surface normal, and only marginally
whether they lie on the face the camera cannot see. Two things decide. How widely the
anchors cover the surface, which a grasp does not control; and whether the measurements
can be placed in the frame in which the generator produces its shape --- and that frame,
we show, is not a function of the input. It is drawn with the noise.
...
\begin{enumerate}
  ...
  $+3.17$ realised against $+7.91$ with the frame given, on shared unanchored passes,
  replicated by an independent campaign at $+3.21$ / $+7.82$
  ...
  manipulations and a $2\times2$ design over 126 paired cases show that spread and
  placement accuracy matter --- spread alone doubles the gain --- while count, normals
  and which face the anchors lie on matter little; the depth-camera control, points
  covering the visible face, outgains tactile patches on the hidden face
...
cast from outside, first hits only), restriction to the face hidden from the camera that
rendered the image ($85\%$ of the anchors are hidden from it, $25$ to $98\%$ depending
on the object; an earlier draw from a misplaced camera reached $61\%$, see
Limitations), and
...
the measurements. For Sec.~\ref{sec:res-anchor} the same model is also run on the
visible face, and uniform draws are taken on either face, so that spread and face can
be crossed.
...
distance and connectivity. For the anchor-set design of Sec.~\ref{sec:res-anchor}, also
the recall of the reference surface hidden from the rendering camera and of the surface
it sees, split by hidden-point removal.
...
\begin{table}[t]
\centering
\caption{Constraint anchoring with tactile contacts, 42 YCB objects $\times$ 3 seeds.
Both arms anchor the same unanchored passes. Last row: an earlier, independent
campaign whose palpation sites were drawn from a misplaced camera (Limitations).}
\label{tab:main}
\begin{tabular}{lr}
\toprule
Unanchored baseline (image only) & 52.7 \Fs \\
\midrule
Mean gain, frame estimated & \gd{$+3.17$} \\
Median / std.\ dev. & $+1.62$ / $8.18$ \\
Positive cases & \gd{78 / 126} \\
Sign test (two-sided) & \gd{$p = 9.5\times10^{-3}$} \\
Objects with positive mean gain & 30 / 42 \\
\midrule
Mean gain, frame given (oracle) & $+7.91$ \\
Positive cases & 111 / 125 \\
Frame cost, paired (oracle $-$ estimated) & $+4.74 \pm 0.68$ \\
\midrule
Replication, earlier draw: estimated / oracle & $+3.21$ / $+7.82$ \\
\bottomrule
\end{tabular}
\end{table}
...
at 42 objects the effect is stronger, so it was not a single object carrying
everything. It also replicates: an independent 42-object campaign, with its own
unanchored passes and palpation sites drawn from a misplaced camera (Limitations),
measured $+3.21$ ($83/125$, $p = 3.1\times10^{-4}$) and $+7.82$ with the frame given;
case by case, the two campaigns differ by $-0.04 \pm 0.70$ and $+0.10 \pm 0.63$.

Comparing the realised gain to the oracle isolates the cost of estimating the frame.
Both arms anchor the same unanchored pass, so the comparison is paired on the anchored
scores: $+4.74 \pm 0.68$ ($102/126$, $p = 10^{-12}$), i.e.\ the estimated frame realises
$40\%$ of the ceiling; under the five-object sweep campaign, $+1.79$ against $+6.77$.
\textbf{Frame estimation absorbs about 60\% of what constraint anchoring can deliver
from a hand's contacts} --- more than any other factor a hand controls
(Sec.~\ref{sec:res-anchor}).
...
the same object and the same noise seed ($n=15$; the visible-face row comes from the
$2\times2$ design of Table~\ref{tab:factorial}, $n=126$, with its own standard error of
$0.60$). Error bars are one standard error
...
camera can see of them. \textbf{(b)} The grasp-simulator result on its own, over the
45 cases of the grasp-strategy campaign, with visibility decided from the camera that
...
anchoring, on five objects (42 for the visible-face row), oracle placement. Three clear
a two-sided sign test, and they share a theme.
...
\begin{table}[t]
\centering
\caption{Anchor set, $2\times2$: gain in \Fs{} over one shared unanchored pass, oracle
frame, 42 objects $\times$ 3 seeds, 128 anchors in every cell. In brackets, the fraction
of each set hidden from the rendering camera.}
\label{tab:factorial}
...
\midrule
8 palpation patches & $+7.91$ \;[0.85] & $+7.31$ \;[0.06] \\
Uniform coverage & \gd{$+18.24$} \;[0.64] & $+15.57$ \;[0.01] \\
\midrule
Spread: uniform $-$ patches & $+10.33$ (115/125) & $+8.26$ (104/124) \\
Face: hidden $-$ visible, patches & \multicolumn{2}{c}{$+0.60$, $p = 0.47$} \\
Face: hidden $-$ visible, uniform & \multicolumn{2}{c}{$+2.67$, $p = 9\times10^{-7}$} \\
\bottomrule
\end{tabular}}
\end{table}

\textbf{Spread dominates, and a hand does not choose it.} A $2\times2$ design over the
42 objects and three seeds, oracle frame, one shared unanchored pass
(Table~\ref{tab:factorial}), crosses how the 128 anchors spread --- eight palpation
patches, or uniform coverage --- with the face they lie on. At equal count, uniform
coverage gains $+10.3$ more than patches on the hidden face and $+8.3$ more on the
visible face ($p < 10^{-14}$ both): spread alone doubles the gain, and a grasp is
patches. The face matters little in comparison: nothing between patches ($+0.6 \pm 0.6$,
$p = 0.47$), $+2.7 \pm 0.5$ in favour of the hidden face under uniform coverage
($p = 9\times10^{-7}$; that draw is only $64\%$ hidden, so this is a lower bound).
Where the gain lands follows the same pattern. With visibility decided from the
rendering camera, hidden-face anchors improve the recall of the hidden side more than
visible-face anchors do --- $+6.7 \pm 1.0$ under uniform coverage, $+1.2 \pm 0.8$ with
patches --- and the visible side about equally. The premise thus survives at the level
of recall, by a margin the spread effect dwarfs. The grasp simulator agrees: three
strategies placing the same six anchors with $87\%$, $60\%$ and $30\%$ of them hidden
gain $+1.37$, $+1.73$ and $+2.55$, none separable from the others, and over 45 cases the
correlation between hidden fraction and gain is $r = -0.001$ (Fig.~\ref{fig:results}b).

\textbf{The depth-camera control.} The visible-face, uniform cell is what a depth
camera delivers: 128 points on the face the camera sees, none on the face it cannot.
It gains $+15.6$ --- twice the $+7.9$ of eight tactile patches on the hidden face
($+7.7 \pm 1.0$ paired, $103/125$, $p = 10^{-13}$). The constraint route therefore
needs no touch to work; touch would outgain depth only where it covered the object as
widely, which one grasp does not. Sparse anchors do not complete what vision misses.
\textbf{They fix global position, scale and orientation of a prior whose frame is
loose, and for that, covering the surface matters far more than which side they lie
on.}
...
$(2.2, -2.2, 1.8)$ renders it. Our first occlusion computations placed that camera in
the mesh's own frame instead, $68^\circ$ from where it rendered: projected from there,
the reference silhouettes overlap the input images at $0.51$ on average over 42
objects, against $0.95$ with the turn applied, and the visible/hidden label differs on
$26\%$ of the surface. We found it late, and redrew. Table~\ref{tab:main},
Table~\ref{tab:factorial} and the recall of Sec.~\ref{sec:res-anchor} use anchors drawn
from the rendering camera; the grasp-strategy fractions of Fig.~\ref{fig:results}b are
recomputed from it (from the misplaced camera the correlation read $-0.145$, and three
cases seemed to leave no anchor on the hidden face). Two artefacts keep the error. The
five-object sweeps of Fig.~\ref{fig:results}a and Table~\ref{tab:hand} use palpation
sites that are $61\%$ hidden from the rendering camera rather than $85\%$; every
comparison there is between two sets drawn the same way, so the error sits on both
sides of each difference. And the 42-object campaign reported as a replication in
Table~\ref{tab:main} used those same sites; that its gains match the redrawn ones to
$-0.04 \pm 0.70$ says that a mostly hidden set and a hidden-face set behave alike,
which Table~\ref{tab:factorial} confirms directly. A smaller defect adds to it: the
renderer normalises objects to a maximum dimension of $1.3$ where the occlusion code
uses $2.0$; with the camera corrected, it changes the label of $3$ of the $45$ grasp
cases.

\textbf{Two caveats of the $2\times2$ design.} Its uniform hidden-face draw is only
$64\%$ hidden, so the face effect under uniform coverage is a lower bound. And on 17 of
the 126 cases a few anchors fall outside the generator's grid after placement and are
ignored by the guidance; dropping those cases changes no conclusion (spread
$+10.5$ / $+8.4$, face $+0.7$ / $+2.7$).
...
A single-view 3D generative prior can be anchored by sparse metric measurements. Of
the measurements themselves, one property matters: how widely they cover the surface
--- uniform coverage gains twice what eight contact patches gain --- and a hand does not
choose it. Their number does not matter over a $16\times$ range; their normals do not
measurably help; which face they lie on matters little, and a depth camera's points on
the visible face outgain tactile patches on the hidden one. What limits a hand's
measurements, once drawn, is whether they can be placed in the frame the generator
produces in --- and that frame is drawn with the noise, so no estimator can learn it.
Estimating it anyway costs 60\% of the achievable gain, and it is what varies between runs: with the frame frozen, anchoring adds no variance on any of four objects.
...
to make, and a more general one: it applies to touch, to depth --- which, covering the
surface, does it better --- and to any sparse metric signal a robot can bring.
