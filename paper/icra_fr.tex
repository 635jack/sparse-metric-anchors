...
\newcommand{\rlblVis}{ancres sur la face visible plutôt}
...
\newcommand{\rlblXlabB}{part d'ancres cachées de la caméra}
...
pré-entraîné. Des mesures elles-mêmes, seule compte vraiment l'étendue avec laquelle
elles couvrent la surface, et une prise ne la choisit pas ; leur nombre, leurs normales
et la face de l'objet qu'elles touchent comptent peu. Ce qui les limite, une fois
tirées, est le \emph{repère de sortie} du générateur. Sur un
...
rééchantillonnée avec la graine de bruit, et se déplace d'environ $135^\circ$ en médiane
...
l'échantillonnage, sans entraînement, améliore le F-Score@2\% de $+3{,}2$ sur 42
objets YCB (deux campagnes indépendantes : $+3{,}17$, $p = 9{,}5\times10^{-3}$, et
$+3{,}21$, $p = 3{,}1\times10^{-4}$ ; test des signes), contre $+7{,}9$ lorsque le
repère est donné : l'estimation du repère consomme 60~\% du gain atteignable, et, une fois le
repère gelé, la passe ancrée est aussi reproductible que la passe non ancrée sur
chaque objet testé. Ancrer le repère par conditionnement, en
...
contrôlées et un plan $2\times2$ à 126 cas appariés, ce qu'un jeu d'ancres exige est
l'étalement spatial et la justesse du placement : à nombre égal, une couverture
uniforme gagne deux fois ce que gagnent huit plaques de contact, tandis que le nombre,
les normales et la face touchée comptent peu --- 128 points de type profondeur sur la
face visible gagnent $+15{,}6$ contre $+7{,}9$ pour huit plaques tactiles sur la face
cachée. Cinq formulations de fusion alternatives qui traitent les mesures comme un
...
$19{,}9$ \Fs{} contre $56{,}5$ pour l'image seule.
\end{abstract}
...
significativement. Mais ce qui décide de combien ils aident n'a presque rien à voir
avec ce qu'une main peut faire varier : ni leur nombre sur une plage de $16\times$, ni
la présence d'une normale, et à peine le fait qu'ils soient sur la face que la caméra
ne voit pas. Deux choses décident. L'étendue avec laquelle les ancres couvrent la
surface, qu'une prise ne contrôle pas ; et la possibilité de placer les mesures dans le
repère où le générateur produit sa forme --- et ce repère, nous le montrons, n'est pas
une fonction de l'entrée. Il est tiré avec le bruit.
...
\begin{enumerate}
  ...
  déplace d'environ $135^\circ$ en médiane sur 42 objets, les objets asymétriques n'étant pas
  ...
  $+3{,}17$ réalisé contre $+7{,}91$ repère donné, sur des passes non ancrées
  partagées, et répliqué par une campagne indépendante à $+3{,}21$ / $+7{,}82$
  ...
  manipulations contrôlées et un plan $2\times2$ à 126 cas appariés montrent que
  l'étalement et la justesse du placement comptent --- l'étalement seul double le gain
  --- tandis que le nombre, les normales et la face touchée comptent peu ; le contrôle
  « caméra de profondeur », des points couvrant la face visible, surpasse des plaques
  tactiles sur la face cachée (Sec.~\ref{sec:res-anchor}).
  \item Une \textbf{décomposition de la variance} d'une exécution à l'autre, montrant
  que, repère gelé, le guidage n'ajoute aucune variance sur aucun de quatre objets, et
  que ce qui domine est donc la réestimation du repère, ainsi que trois pièges
...
objets, l'écart médian entre graines vaut $136{,}5^\circ \pm 3{,}7$ pour les objets
asymétriques et $135{,}7^\circ \pm 3{,}7$ pour les symétriques, moyenne et écart-type
sur quatre exécutions indépendantes du recalage ; dans chacune, 1 objet asymétrique sur
16 est stable sous $30^\circ$ et 0 sur 26 chez les symétriques ; la corrélation
asymétrie / stabilité vaut $-0{,}14 \pm 0{,}06$. Le recalage échantillonne des points
au hasard, et sur un solide de révolution la rotation qu'il trouve est arbitraire,
d'où la dispersion des médianes ; la conclusion au niveau de la partition, elle, ne
bouge pas --- les comptes d'objets stables et l'absence de toute relation
asymétrie--stabilité sont identiques dans les quatre exécutions. (La mesure
d'asymétrie par objet est elle-même stable à $0{,}001$ ; un objet est posé sur le seuil
de $0{,}02$ et change de côté dans une exécution sur quatre, donnant 17/25.) Le partage par symétrie écarte un artefact de mesure --- deux sorties
...
de crête mémoire, $\approx 55$\,s par échantillon à 20 pas sur un M2 Max. Les poids de
l'ossature sont utilisés tels quels, mais son code n'est pas celui publié. La faire
tourner sur Metal a demandé de convertir des tenseurs float64 en float32 à plusieurs
endroits et de compiler nativement la bibliothèque de convolutions éparses ; une
rétropropagation à travers le modèle ne tient en mémoire qu'avec du \emph{gradient
checkpointing} sur le transformeur de diffusion (crête $21 \rightarrow 6{,}5$\,Go) ; et
trois pull requests du dépôt public y sont fusionnées. Le guidage, lui, enveloppe le
débruiteur au moment de l'échantillonnage et ne modifie ni l'échantillonneur ni les
poids. Les campagnes d'ancrage rapportées ici ont toutes tourné sur ce portage, et
aucune n'a été recoupée avec l'implémentation CUDA de référence. Le code modifié est
publié avec l'article, accompagné d'une notice qui liste ces changements comme l'exige
la licence de l'ossature.
...
impacts seuls), restriction à la face cachée de la caméra qui a rendu l'image
($85$~\% des ancres en sont cachées, de $25$ à $98$~\% selon l'objet ; un premier
tirage depuis une caméra mal placée n'atteignait que $61$~\%, voir les limites), et
rejet des points où
...
mesures. Pour la Sec.~\ref{sec:res-anchor}, le même modèle est aussi appliqué à la face
visible, et des tirages uniformes sont pris sur l'une et l'autre face, de façon à
croiser étalement et face.
...
symétrique et connectivité. Pour le plan sur le jeu d'ancres de la
Sec.~\ref{sec:res-anchor}, aussi le rappel de la surface de référence cachée de la
caméra du rendu et de celle qu'elle voit, séparées par suppression des points cachés.
...
\begin{table}[t]
\centering
\caption{Ancrage par contrainte avec des contacts tactiles, 42 objets YCB $\times$ 3
graines. Les deux bras ancrent les mêmes passes non ancrées. Dernière ligne : une
campagne antérieure et indépendante, dont les sites de palpation venaient d'une caméra
mal placée (limites).}
\label{tab:main}
\begin{tabular}{lr}
\toprule
Témoin non ancré (image seule) & 52,7 \Fs \\
\midrule
Gain moyen, repère estimé & \gd{$+3{,}17$} \\
Médiane / écart-type & $+1{,}62$ / $8{,}18$ \\
Cas positifs & \gd{78 / 126} \\
Test des signes (bilatéral) & \gd{$p = 9{,}5\times10^{-3}$} \\
Objets à gain moyen positif & 30 / 42 \\
\midrule
Gain moyen, repère donné (oracle) & $+7{,}91$ \\
Cas positifs & 111 / 125 \\
Coût du repère, apparié (oracle $-$ estimé) & $+4{,}74 \pm 0{,}68$ \\
\midrule
Réplication, tirage antérieur : estimé / oracle & $+3{,}21$ / $+7{,}82$ \\
\bottomrule
\end{tabular}
\end{table}
...
$+1{,}79$ à $p = 0{,}035$ ; à 42 objets l'effet est plus fort --- ce n'était donc pas
un objet qui portait tout. Il se réplique aussi : une campagne indépendante à 42
objets, avec ses propres passes non ancrées et des sites de palpation tirés depuis une
caméra mal placée (limites), mesurait $+3{,}21$ ($83/125$, $p = 3{,}1\times10^{-4}$) et
$+7{,}82$ repère donné ; cas par cas, les deux campagnes diffèrent de $-0{,}04 \pm
0{,}70$ et $+0{,}10 \pm 0{,}63$.

Comparer le gain réalisé à l'oracle isole le coût de l'estimation du repère. Les deux
bras ancrent la même passe non ancrée, la comparaison est donc appariée sur les scores
ancrés : $+4{,}74 \pm 0{,}68$ ($102/126$, $p = 10^{-12}$), soit un repère estimé qui
réalise $40$~\% du plafond ; sous la campagne de balayage à cinq objets, $+1{,}79$
contre $+6{,}77$. \textbf{L'estimation du repère absorbe environ 60~\% de ce que
l'ancrage par contrainte peut tirer des contacts d'une main} --- plus que tout autre
facteur qu'une main contrôle (Sec.~\ref{sec:res-anchor}).
...
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{\input{fig_results}}
...
ancrées, sur le même objet et la même graine de bruit ($n=15$ ; la ligne « face
visible » vient du plan $2\times2$ du tableau~\ref{tab:factorial}, $n=126$, avec sa
propre erreur type de $0{,}60$). Les barres d'erreur
valent une erreur type --- $0{,}58$ lorsque les deux conditions viennent de la même
campagne et partagent donc le repère oracle, le bruit de placement s'annulant alors
dans la différence (poolé sur quatre objets), et au moins $0{,}91$ lorsqu'elles viennent
de campagnes distinctes, qui ne partagent aucune passe non ancrée (mesuré sur un objet). La couleur marque la significativité au test des
...
sur ce que la caméra en voit. \textbf{(b)} Le résultat du simulateur de préhension
seul, sur les 45 cas de sa campagne, la visibilité étant décidée depuis la
caméra qui a rendu les images. L'ajustement par moindres carrés est plat.}
\label{fig:results}
\end{figure*}

La Fig.~\ref{fig:results}a balaie sept propriétés du jeu d'ancres sous ancrage par
contrainte, sur cinq objets (42 pour la ligne « face visible »), placement oracle.
Trois passent un test des signes bilatéral, et elles partagent un thème.
...
\begin{table}[t]
\centering
\caption{Jeu d'ancres, plan $2\times2$ : gain en \Fs{} sur une passe non ancrée
partagée, repère oracle, 42 objets $\times$ 3 graines, 128 ancres dans chaque case.
Entre crochets, la part de chaque jeu cachée de la caméra du rendu.}
\label{tab:factorial}
...
\midrule
8 plaques de palpation & $+7{,}91$ \;[0,85] & $+7{,}31$ \;[0,06] \\
Couverture uniforme & \gd{$+18{,}24$} \;[0,64] & $+15{,}57$ \;[0,01] \\
\midrule
Étalement : uniforme $-$ plaques & $+10{,}33$ (115/125) & $+8{,}26$ (104/124) \\
Face : cachée $-$ visible, plaques & \multicolumn{2}{c}{$+0{,}60$, $p = 0{,}47$} \\
Face : cachée $-$ visible, uniforme & \multicolumn{2}{c}{$+2{,}67$, $p = 9\times10^{-7}$} \\
\bottomrule
\end{tabular}}
\end{table}

\textbf{L'étalement domine, et une main ne le choisit pas.} Un plan $2\times2$ sur les
42 objets et trois graines, repère oracle, une passe non ancrée partagée
(tableau~\ref{tab:factorial}), croise la façon dont les 128 ancres s'étalent --- huit
plaques de palpation, ou couverture uniforme --- avec la face où elles se trouvent. À
nombre égal, la couverture uniforme gagne $+10{,}3$ de plus que les plaques sur la face
cachée et $+8{,}3$ de plus sur la face visible ($p < 10^{-14}$ dans les deux cas) :
l'étalement seul double le gain, et une prise, ce sont des plaques. La face compte peu
en comparaison : rien entre plaques ($+0{,}6 \pm 0{,}6$, $p = 0{,}47$), $+2{,}7 \pm
0{,}5$ en faveur de la face cachée sous couverture uniforme ($p = 9\times10^{-7}$ ; ce
tirage n'est caché qu'à $64$~\%, c'est donc une borne inférieure). Là où le gain se
dépose suit le même motif. La visibilité étant décidée depuis la caméra du rendu, des
ancres sur la face cachée améliorent le rappel du côté caché plus que des ancres sur la
face visible --- $+6{,}7 \pm 1{,}0$ sous couverture uniforme, $+1{,}2 \pm 0{,}8$ avec
des plaques --- et le côté visible à peu près autant. La prémisse survit donc au niveau
du rappel, d'une marge que l'effet d'étalement écrase. Le simulateur de préhension
concorde : trois stratégies plaçant les mêmes six ancres avec $87$~\%, $60$~\% et
$30$~\% d'entre elles cachées gagnent $+1{,}37$, $+1{,}73$ et $+2{,}55$, sans qu'aucune
se sépare des autres, et sur 45 cas la corrélation entre part cachée et gain vaut
$r = -0{,}001$ (Fig.~\ref{fig:results}b).

\textbf{Le contrôle « caméra de profondeur ».} La case face visible, couverture
uniforme, est ce qu'une caméra de profondeur fournit : 128 points sur la face que la
caméra voit, aucun sur celle qu'elle ne voit pas. Elle gagne $+15{,}6$ --- deux fois les
$+7{,}9$ de huit plaques tactiles sur la face cachée ($+7{,}7 \pm 1{,}0$ apparié,
$103/125$, $p = 10^{-13}$). La voie par contrainte n'a donc besoin d'aucun toucher pour
fonctionner ; le toucher ne surpasserait la profondeur que là où il couvrirait l'objet
aussi largement, ce qu'une prise ne fait pas. Les ancres éparses ne complètent pas ce
que la vision manque. \textbf{Elles fixent position, échelle et orientation globales
d'un a priori dont le repère est lâche, et pour cela, couvrir la surface compte bien
plus que le côté où elles se trouvent.}
...
\begin{table}[t]
\centering
\caption{Six contacts orientés, avec et sans a priori génératif. Les valeurs Poisson sont moyenne $\pm$ écart-type sur cinq exécutions.}
\label{tab:poisson}
\begin{tabular}{lr}
\toprule
Reconstruction de Poisson~\cite{poisson}, 6 contacts orientés & $19{,}90 \pm 0{,}36$ \Fs \\
Reconstruction de Poisson, 18 contacts (3 prises) & $20{,}44 \pm 0{,}18$ \\
\midrule
...
Elle n'atteint pourtant que 35~\% de ce que l'image seule obtient déjà. Les mesures
...
($0{,}70$ contre $0{,}68$) : sur cet objet, \textbf{91~\% de la variance d'un score ancré
vient de la réestimation du placement des ancres}. Refaire la mesure à repère gelé sur
trois objets de plus donne des écarts-types ancrés de $1{,}74$, $0{,}40$ et $2{,}56$
contre $2{,}53$, $0{,}49$ et $2{,}19$ non ancrés --- la passe ancrée n'est jamais plus
bruitée que la passe non ancrée, sur aucun objet. \textbf{Le guidage n'amplifie rien.}
Ce qui varie, c'est l'objet : le plancher va de $0{,}40$ à $2{,}56$ sur les quatre, un
facteur six. Une bonne part vient de la notation,
pas du générateur. Noter quatre fois chaque maillage enregistré, sans rien régénérer,
donne des écarts-types de $0{,}33$ à $2{,}23$ selon l'objet. Regroupée sur les quatre,
la notation seule a un écart-type de $1{,}45$ quand un score ancré en a $1{,}60$, et de
$1{,}03$ quand un score non ancré en a $1{,}72$ : le recalage comme le F-Score
échantillonnent des points au hasard, et l'ICP tombe parfois dans un autre bassin,
jusqu'à $8$ points d'écart sur un même maillage. À repère gelé, la génération elle-même
ajoute peu (au plus $0{,}66$, par différence). Nos erreurs types viennent de notations
uniques et incluent donc ce bruit ; moyenner plusieurs notations par prédiction les
réduirait. Le
problème de repère de la Sec.~\ref{sec:frame} apparaît donc deux fois : il consomme
l'essentiel du gain moyen, et il domine la variance.
...
s'annule pas, aucune ne partageant sa passe non ancrée. Poolé sur les quatre objets,
le $\sigma$ à repère gelé d'un score ancré vaut $1{,}60$, ce qui met l'erreur type
intra-campagne d'une différence appariée à $0{,}58$ sur 15 cas ; le chiffre
inter-campagnes, $0{,}91$, n'a été mesuré que sur un objet et est une borne inférieure
(Fig.~\ref{fig:results}a). Sous ces valeurs, les deux dégradations sont à $4$ erreurs
types et plus, et les deux effets que le test des signes rejette --- normales,
visibilité --- entre $1{,}4$ et $1{,}6$. Nous fondons néanmoins chaque affirmation sur
le test des signes, qui ne demande aucune estimation de $\sigma$ et rend le même
verdict.
...
\textbf{Une caméra placée dans le mauvais repère.} L'importateur OBJ de Blender tourne
chaque maillage Y-up dans son monde Z-up, $(x, y, z) \mapsto (x, -z, y)$, avant que la
caméra en $(2{,}2\,; -2{,}2\,; 1{,}8)$ ne le rende. Nos premiers calculs d'occultation
ont placé cette caméra dans le repère propre du maillage, à $68^\circ$ de celle du
rendu : projetées depuis là, les silhouettes de référence recouvrent les images
d'entrée à $0{,}51$ en moyenne sur 42 objets, contre $0{,}95$ avec la rotation, et
l'étiquette visible/cachée diffère sur $26$~\% de la surface. Nous l'avons trouvé tard,
et retiré les ancres. Le tableau~\ref{tab:main}, le tableau~\ref{tab:factorial} et le
rappel de la Sec.~\ref{sec:res-anchor} utilisent des ancres tirées depuis la caméra du
rendu ; les parts cachées des stratégies de préhension et la Fig.~\ref{fig:results}b
sont recalculées depuis elle (depuis la caméra mal placée, la corrélation valait
$-0{,}145$ et trois cas semblaient ne laisser aucune ancre sur la face cachée). Deux
artefacts gardent l'erreur. Les balayages à cinq objets de la Fig.~\ref{fig:results}a
et du tableau~\ref{tab:hand} utilisent des sites de palpation cachés à $61$~\% de la
caméra du rendu plutôt qu'à $85$~\% ; chaque comparaison y oppose deux jeux tirés de la
même façon, l'erreur est donc des deux côtés de chaque différence. Et la campagne à 42
objets rapportée comme réplication dans le tableau~\ref{tab:main} utilisait ces mêmes
sites ; que ses gains rejoignent ceux des ancres retirées à $-0{,}04 \pm 0{,}70$ dit
qu'un jeu majoritairement caché et un jeu de face cachée se comportent de même, ce que
le tableau~\ref{tab:factorial} confirme directement. Un défaut plus petit s'y ajoute :
le rendu normalise les objets à une dimension maximale de $1{,}3$ quand le code
d'occultation utilise $2{,}0$ ; caméra corrigée, il change l'étiquette de $3$ des $45$
cas de préhension.

\textbf{Deux réserves sur le plan $2\times2$.} Son tirage uniforme sur la face cachée
n'est caché qu'à $64$~\%, l'effet de face sous couverture uniforme est donc une borne
inférieure. Et sur 17 des 126 cas, quelques ancres tombent hors de la grille du
générateur après placement et sont ignorées par le guidage ; retirer ces cas ne change
aucune conclusion (étalement $+10{,}5$ / $+8{,}4$, face $+0{,}7$ / $+2{,}7$).
...
Des mesures elles-mêmes, une propriété compte : l'étendue avec laquelle elles couvrent
la surface --- une couverture uniforme gagne deux fois ce que gagnent huit plaques de
contact --- et une main ne la choisit pas. Leur nombre n'a pas d'effet sur une plage de
$16\times$ ; leurs normales n'aident pas de façon mesurable ; la face où elles se
trouvent compte peu, et les points d'une caméra de profondeur sur la face visible
surpassent des plaques tactiles sur la face cachée. Ce qui limite les mesures d'une
main, une fois tirées, est de savoir si elles peuvent être placées dans le repère où
le générateur produit --- et ce repère est tiré avec le bruit, si bien qu'aucun
estimateur ne peut l'apprendre. L'estimer malgré tout coûte 60~\% du gain atteignable et explique ce qui varie d'une exécution à l'autre : repère gelé, l'ancrage n'ajoute aucune variance sur aucun de quatre objets.
...
profondeur --- qui, couvrant la surface, le fait mieux --- et pour tout signal métrique
épars qu'un robot peut apporter.
