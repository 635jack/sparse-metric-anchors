...
def get_fused_voxel_image_data(
    image_file,
    voxel_file,
    image_transform,
    device,
    voxel_resolution=16,
    image_over_white=None,
):
    """Helper pour le mode fusion multimodale : charge en parallèle une image
    RGB et un fichier voxel JSON du même objet.

    Le dict retourné contient les deux entrées :
      - data["images"]  : (1, 3, H, W) float tensor — image RGB normalisée
      - data["voxels"]  : (1, 1, R, R, R) float tensor — grille d'occupance
      - data["img_idx"] : (1,) long tensor — index de vue (0 = vue frontale)
      - data["low"]     : (1, 1, 46, 46, 46) float tensor — placeholder diffusion
      - data["id"]      : [str] identifiant de l'objet

    Args:
        image_file       : chemin vers l'image RGB (str ou file-object)
        voxel_file       : chemin vers le JSON voxel (str)
        image_transform  : transform torchvision (Resize+CenterCrop+Normalize)
        device           : torch.device
        voxel_resolution : résolution de la grille voxel (16 pour WaLa-VX16)
        image_over_white : comportement pour la transparence (voir load_image)
    """
    # ---- Identifiant commun ----
    if isinstance(image_file, str):
        file_base_name = osp.basename(image_file).split(".")[0]
    ...
    image = load_image(image_file, image_over_white=image_over_white)
    image = image_transform(image)
    images = image.to(device).unsqueeze(0).float()  # (1, 3, H, W)
    img_idx = torch.from_numpy(np.array([0])).long().to(device)
    ...
    actual_resolution = voxel_json.get("resolution", voxel_resolution)
    occupancy_arr = np.array(voxel_json["occupancy"])
    color_numpy = np.array(voxel_json["color"])
    ...
    )[:, :, :, 3:4]  # canal alpha = occupance binaire

    voxel_grid = voxel_grid.transpose(3, 0, 1, 2).astype(np.float32)
    voxel_grid = torch.from_numpy(voxel_grid).to(device)
    voxels = voxel_grid.unsqueeze(0).float()  # (1, 1, R, R, R)
...
def get_fused_pointcloud_image_data(
    image_file,
    pointcloud_file,
    image_transform,
    device,
    ...
    """Mode fusion image + nuage de points.

    Pendant de `get_fused_voxel_image_data`, avec le nuage comme modalité
    géométrique au lieu de la grille d'occupation. Le nuage est lu depuis un
    tenseur .pt de forme (1, N, 3) ou (N, 3), tel que produit par
    tools/make_pointclouds.py.

    Le dict retourné contient :
      - data["images"]     : (1, 3, H, W)
      - data["Pointcloud"] : (1, N, 3)
      - data["img_idx"]    : (1,)
      - data["low"]        : (1, 1, 46, 46, 46) placeholder diffusion
      - data["id"]         : [str]
    """
    if isinstance(image_file, str):
        file_base_name = osp.basename(image_file).split(".")[0]
    ...
    points = torch.load(pointcloud_file, map_location="cpu", weights_only=True)
    if points.dim() == 2:
        points = points.unsqueeze(0)
    points = points.to(device).float()  # (1, N, 3)
