...
class get_model(nn.Module):
    ...
    def training_losses(self, data_input, condition=None, image_index=None,
        ...
        condition = self.process_condition(condition, image_index=image_index)
        # `condition_global` alimente les branches temporelle et résiduelle, qui
        # moyennent les tokens ; `condition` alimente la cross-attention, qui seule
        # préserve la structure spatiale. Voir Latent_UVIT.forward.
        cond_global = (
            self.process_condition(condition_global, image_index=image_index)
            if condition_global is not None else None
        )
        loss = self.diffusion_module.training_losses(
            self.unet, x_start=data_input, t=t, latent_codes=condition,
            latent_codes_global=cond_global,
        )
