...
class Latent_UVIT(nn.Module):
    ...
    def forward(self, x, t, latent_codes=None, latent_codes_global=None):
        """
        Forward pass of DiT.
        ...
        # Découplage des branches de conditionnement.
        #
        # Trois branches consomment `latent_codes`, et seule la première préserve la
        # structure spatiale :
        #   - cross-attention  : les tokens entiers, en contexte des blocs transformer ;
        #   - temporelle       : Global_Proj_Layer -> torch.mean sur les tokens ;
        #   - résiduelle conv  : Global_Proj_Layer -> torch.mean sur les tokens.
        # Global_Proj_Layer compresse 256x1024 en 128 scalaires, soit un facteur 2048 :
        # toute information de position y est perdue.
        #
        # En fusion, injecter la géométrie dans les branches globales décale le
        # conditionnement d'ensemble sans pouvoir corriger un détail localisé — une
        # perturbation globale déplace toute la forme, elle ne répare pas une anse.
        # `latent_codes_global`, quand il est fourni, alimente les deux branches
        # moyennées (typiquement les features image seules) pendant que `latent_codes`
        # alimente la cross-attention (les features fusionnées).
        codes_global = latent_codes if latent_codes_global is None else latent_codes_global

        if self.add_condition_res_ch is not None:
            res_cond_emb = self.res_cond_proj_layer(codes_global)
        else:
            res_cond_emb = None
        ...
        if self.add_condition_time_ch and self.context_dim is not None:
            global_context = self.global_proj(codes_global)
            att_t = t + global_context
            # print(att_t.shape)
        else:
            att_t = t

        # Gradient checkpointing : recalcule les activations pendant le backward au
        # lieu de les conserver. Activé seulement à l'entraînement (gradient requis)
        # et jamais à l'inférence, où il n'apporterait rien.
        #
        # Dans le fine-tuning de la fusion, le DiT est gelé : on n'a besoin du
        # backward que pour remonter jusqu'au conditionnement. Autograd stocke
        # pourtant toutes les activations intermédiaires, d'où un pic de 20,7 Go —
        # au-delà de ce qu'un Mac 32 Go peut fournir une fois macOS et les
        # applications servies. Le checkpointing ramène ce pic autour de 8 Go,
        # contre environ 30 % de temps de calcul en plus.
        use_ckpt = getattr(self, "gradient_checkpointing", False) and torch.is_grad_enabled()
        if use_ckpt:
            from torch.utils.checkpoint import checkpoint as _ckpt

            def _run(module, *a, **kw):
                return _ckpt(module, *a, use_reentrant=False, **kw)
        else:
            def _run(module, *a, **kw):
                return module(*a, **kw)
        ...
        for module in self.input_blocks:
            h = _run(module, h, t, res_emb=res_cond_emb)
            hs.append(h)
        ...
        for block in self.blocks:
            h, latent_codes_local = _run(block, h, att_t, context=latent_codes_local)
        # print(h.shape)
        if self.add_num_register > 0:
            # print(h.shape)
        ...
        for module in self.output_blocks:
            skip_h = hs.pop()
            h = _run(module, h, t, skip_h=skip_h, res_emb=res_cond_emb)
